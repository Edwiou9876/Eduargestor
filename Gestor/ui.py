...
class CreateClientWindow(Toplevel, CenterWidgetMixin):
    ...
    def build(self):
        ...
        Label(frame, text="DNI (2 ints y un uperchar)").grid(row=0,column=0)
        Label(frame, text="Nombre (2 a 30 char)").grid(row=0,column=1)
        Label(frame, text="Apellido (2 a 30 char)").grid(row=0,column=2)

        dni = Entry(frame)
        dni.grid(row=1,column=0)
        dni.bind("<KeyRelease>", lambda event: self.valiate(event, 0))
        nombre = Entry(frame)
        nombre.grid(row=1,column=1)
        nombre.bind("<KeyRelease>", lambda event: self.valiate(event,1))
        apellido = Entry(frame)
        apellido.grid(row=1,column=2)
        apellido.bind("<KeyRelease>", lambda event: self.valiate(event,2))
    ...
    def valiate(self,event,index):
        valor = event.widget.get()

        valido = helpers.dni_validacion(valor,db.Clientes.lista) if index == 0 else valor.isalpha() and len(valor) >= 3 and len(valor) <= 30
...
class EditClientWindow(Toplevel, CenterWidgetMixin):
    ...
    def build(self):
        ...
        nombre = Entry(frame)
        nombre.grid(row=1,column=1)
        nombre.bind("<KeyRelease>", lambda event: self.valiate(event,0))
        apellido = Entry(frame)
        apellido.grid(row=1,column=2)
        apellido.bind("<KeyRelease>", lambda event: self.valiate(event,1))
    ...
    def valiate(self,event,index):
        ...
        self.validaciones[index] = valido
        self.actualizar.config(state=NORMAL if self.validaciones== [True,True] else DISABLED)
